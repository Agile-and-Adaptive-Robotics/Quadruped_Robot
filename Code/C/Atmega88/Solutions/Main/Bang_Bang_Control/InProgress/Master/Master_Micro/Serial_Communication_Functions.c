...
void serial_write_int2matlab( uint16_t myint )
{
	...
	uint8_t * int_bytes = uint162lowhighbytes( myint );
...
void serial_write_int_array2matlab( uint16_t myint_array[], uint8_t array_length )
{
	...
	for (int k1 = 0; k1 < array_length; ++k1)
	{
		...
		// Convert the integer into its high and low bytes.
		int_bytes = uint162lowhighbytes( myint_array[k1] );
...
void serial_write_single_array2matlab( float myfloat_array[], uint8_t array_length )
{
	...
	uint8_t float_bytes[NUM_BYTES_PER_FLOAT];
...
void serial_write_sensor_data_ints2matlab( struct int_array_struct * sensor_data_ptr )
{
	...
	volatile uint8_t int_bytes_temp[NUM_BYTES_PER_UINT16];
	volatile uint8_t id_byte_temp;
	uint16_t check_sum;
	...
	for (int k1 = 0; k1 < sensor_data_ptr->length; ++k1)
	{
		...
		uint162byte_array( sensor_data_ptr->values[k1], int_bytes_temp );
...
void serial_write_sensor_data_singles2matlab( struct single_array_struct * sensor_data_ptr )
{
	...
	// Preallocate an array to store the bytes of each float.
	uint8_t float_bytes_temp[NUM_BYTES_PER_FLOAT];
	uint8_t id_byte_temp;
...
void wait_for_start_sequence( void )
{
	...
	uint8_t match_found;
	uint8_t byte_window[WINDOW_SIZE];
...
uint16_t serial_read_matlab_int( void )
{
	...
	myint = lowhighbytes2uint16( low_byte, high_byte );
...
struct int_struct serial_read_matlab_int_array( void )
{
	...
	uint8_t num_ints;							// This assumes that the number of integers being sent is < 128.
	uint8_t low_byte;
	uint8_t high_byte;
	static uint16_t array[24];					// Define an array to store the incoming integers.
	...
	for ( int k = 0; k < num_ints; ++k)
	{
		...
		array[k] = lowhighbytes2uint16( low_byte, high_byte );
	}
...
struct float_struct serial_read_matlab_single_array( void )
{
	...
	uint8_t num_singles;					// This assumes that the number of singles being sent is < 128.
	static float array[24];					// Define an array to store the incoming integers.
	static uint8_t float_bytes[NUM_BYTES_PER_FLOAT];
...
void serial_read_matlab_muscle_command_ints( struct int_array_struct * command_data_ptr )
{
	...
	// Define local variables.
	uint8_t num_commands;										// This assumes that the number of ints being sent is <255.
	static uint8_t int_bytes[NUM_BYTES_PER_UINT16];
	...
	for ( int k1 = 0; k1 < num_commands; ++k1)
	{
		...
		command_data_ptr->values[k1] = byte_array2uint16( int_bytes );
	}
...
void serial_read_matlab_muscle_command_singles( struct single_array_struct * command_data_ptr )
{
	...
	// Define local variables.
	uint8_t num_commands;					// This assumes that the number of singles being sent is <255.
	static uint8_t float_bytes[NUM_BYTES_PER_FLOAT];
