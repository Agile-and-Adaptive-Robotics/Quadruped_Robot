function [muscle_values, ts, num_commands] = GenerateSinusoidalCommands(num_muscles, num_cycles, num_per_cycle, on_value)
...
muscle_values(:, 1:2:end) = (on_value/2)*(sin((2*pi/num_per_cycle)*ts - pi/2)'*ones(1, num_muscles/2) + 1);
muscle_values(:, 2:2:end) = (on_value/2)*(sin((2*pi/num_per_cycle)*ts + pi/2)'*ones(1, num_muscles/2) + 1);
