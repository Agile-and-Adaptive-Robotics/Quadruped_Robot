%% Division Subnetwork Conversion Example.
...
%% Define the Fundamental Parameters of a Relative Division Subnetwork.
...
c3_relative = 20e-9;                                                                                    % [S] Design Constant 3.
delta_relative = 1e-3;                                                                                  % [V] Membrane Voltage Offset.


%% Compute the Derived Parameters of a Relative Division Subnetwork.

% Compute the network design parameters.
c1_relative = c3_relative;                                                                                                                                      % [S] Design Constant 1. 
c2_relative = ( R3_relative*c1_relative - delta_relative*c3_relative )/( delta_relative );                                                                      % [S] Design Constant 2.

% Compute the maximum synaptic conductances.
gs31_relative = ( R3_relative*Gm3_relative - Ia3_relative )/( dEs31_relative - R3_relative );                                                                   % [S] Synaptic Conductance (Synapse 31).
gs32_relative = ( ( dEs31_relative - delta_relative )*gs31_relative + Ia3_relative - delta_relative*Gm3_relative )/( delta_relative - dEs32_relative );         % [S] Synaptic Conductance (Synapse 32).
...
c2_absolute = 10*c2_relative;                                                                                   % [A] Design Constant 2.
c3_absolute = ( delta_absolute*c2_absolute*R2_absolute )/( R3_absolute - delta_absolute );                      % [W] Design Constant 3.
c1_absolute = ( R3_absolute*c3_absolute )/( R1_absolute );                                                      % [W] Design Constant 1.
...
% Compute the maximum membrane voltage.
