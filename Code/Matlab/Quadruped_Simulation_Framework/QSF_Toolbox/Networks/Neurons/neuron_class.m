classdef neuron_class
    ...
    methods
        ...
        function self = neuron_class( ID, name, U, h, Cm, Gm, Er, R, Am, Sm, dEm, Ah, Sh, dEh, dEna, tauh_max, Gna, I_leak, I_syn, I_na, I_tonic, I_app, I_total, b_enabled, neuron_utilities )
            ...
            m_inf = self.compute_minf( U, Am, Sm, dEm, neuron_utilities );                              % [-] Steady State Sodium Channel Activation Parameter
            h_inf = self.compute_hinf( U, Ah, Sh, dEh, neuron_utilities );                              % [-] Steady State Sodium Channel Deactivation Parameter
            
            % Store the steady state soduim channel activation and deactivation parameters.
            self.m_inf = m_inf;
            self.h_inf = h_inf;
            ...
            tauh = self.compute_tauh( U, tauh_max, h_inf, Ah, Sh, dEh, neuron_utilities );              % [-] Sodium Channel Deactivation Time Constant
            
            % Store the sodium channel deactivation time constant.
            self.tauh = tauh;                                                                           
        ...
        function self = compute_set_absolute_double_subtraction_Gna( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute and set the sodium channel conductance for absolute double subtraction subnetwork neurons.
            self.Gna = self.compute_absolute_double_subtraction_Gna( neuron_utilities );                                     	% [S] Sodium Channel Conductance
        ...
        function self = compute_set_split_vb_integration_Gna( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute and set the sodium channel conductance for split voltage based integration subnetwork neurons.
            self.Gna = self.compute_split_vb_integration_Gna( neuron_utilities );                                            	% [S] Sodium Channel Conductance
        ...
        % Implement a function to compute the membrance conductance for absolute addition subnetwork input neurons.
        function self = compute_set_absolute_addition_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_addition_Gm_input( neuron_utilities );                                              % [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrance conductance for absolute addition subnetwork output neurons.
        function self = compute_set_absolute_addition_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_addition_Gm_output( neuron_utilities );                                          	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrance conductance for relative addition subnetwork input neurons.
        function self = compute_set_relative_addition_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_addition_Gm_input( neuron_utilities );                                          	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrance conductance for relative addition subnetwork output neurons.
        function self = compute_set_relative_addition_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_addition_Gm_output( neuron_utilities );                                           	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute subtraction subnetwork input neurons.
        function self = compute_set_absolute_subtraction_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_subtraction_Gm_input( neuron_utilities );                                           % [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute subtraction subnetwork output neurons.
        function self = compute_set_absolute_subtraction_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_subtraction_Gm_output( neuron_utilities );                                       	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative subtraction subnetwork input neurons.
        function self = compute_set_relative_subtraction_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_subtraction_Gm_input( neuron_utilities );                                        	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative subtraction subnetwork output neurons.
        function self = compute_set_relative_subtraction_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_subtraction_Gm_output( neuron_utilities );                                        	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute inversion subnetwork input neurons.
        function self = compute_set_absolute_inversion_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_inversion_Gm_input( neuron_utilities );                                             % [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute inversion subnetwork output neurons.
        function self = compute_set_absolute_inversion_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_inversion_Gm_output( neuron_utilities );                                         	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative inversion subnetwork input neurons.
        function self = compute_set_relative_inversion_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_inversion_Gm_input( neuron_utilities );                                           	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative inversion subnetwork output neurons.
        function self = compute_set_relative_inversion_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_inversion_Gm_output( neuron_utilities );                                          	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute division subnetwork input neurons.
        function self = compute_set_absolute_division_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_division_Gm_input( neuron_utilities );                                           	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for absolute division subnetwork output neurons.
        function self = compute_set_absolute_division_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_absolute_division_Gm_output( neuron_utilities );                                             % [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative division subnetwork input neurons.
        function self = compute_set_relative_division_Gm_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_division_Gm_input( neuron_utilities );                                            	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute the membrane conductance for relative division subnetwork output neurons.
        function self = compute_set_relative_division_Gm_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the membrane conductance.
            self.Gm = self.compute_relative_division_Gm_output( neuron_utilities );                                           	% [S] Membrane Conductance
            
        end
        
        
        % Implement a function to compute and set the membrane conductance for a derivation neuron.
        function self = compute_set_derivation_Gm( self, k, w, safety_factor, neuron_utilities )
            ...
            self.Gm = self.compute_derivation_Gm( k, w, safety_factor, neuron_utilities );                                    	% [S] Membrane Conductance
        ...
        function self = compute_set_transmission_Cm( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute and set the membrane capacitance for a transmission subnetwork neuron.
            self.Cm = self.compute_transmission_Cm( neuron_utilities );                                                     	% [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the membrane capacitance for a slow transmission subnetwork neuron.
        function self = compute_set_slow_transmission_Cm( self, Gm, num_cpg_neurons, T, r, neuron_utilities )
            ...
            self.Cm = self.compute_slow_transmission_Cm( Gm, num_cpg_neurons, T, r, neuron_utilities );                     	% [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the membrane capacitance for a modulation subnetwork neuron.
        function self = compute_set_modulation_Cm( self, neuron_utilities )
        ...
        function self = compute_set_derivation_Cm1( self, Gm, Cm2, k, neuron_utilities )
            ...
            self.Cm = self.compute_derivation_Cm1( Gm, Cm2, k, neuron_utilities  );                                             % [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the second membrane capacitance for a derivation subnetwork neuron.
        function self = compute_set_derivation_Cm2( self, Gm, w, neuron_utilities )
            ...
            self.Cm = self.compute_derivation_Cm2( Gm, w, neuron_utilities );                                                   % [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the membrane capacitance for an integration neuron.
        function self = compute_set_integration_Cm( self, ki_mean, neuron_utilities )
            ...
            self.Cm = self.compute_integration_Cm( ki_mean, neuron_utilities );                                                 % [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the membrane capacitance for a voltage based integration neuron.
        function self = compute_set_vb_integration_Cm( self, ki_mean, neuron_utilities )
            ...
            self.Cm = self.compute_vb_integration_Cm( ki_mean, neuron_utilities );                                           	% [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the first membrane capacitance for a split voltage based integration neuron.
        function self = compute_set_split_vb_integration_Cm1( self, ki_mean, neuron_utilities )
            ...
            self.Cm = self.compute_split_vb_integration_Cm1( ki_mean, neuron_utilities );                                    	% [F] Membrane Capacitance
            
        end
        
        
        % Implement a function to compute and set the second membrane capacitance for a split voltage based integration neuron.
        function self = compute_set_split_vb_integration_Cm2( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute and set the second membrane capacitance for this split voltage based integration neuron.
            self.Cm = self.compute_split_vb_integration_Cm2( neuron_utilities );                                              	% [F] Membrane Capacitance
        ...
        % Implement a function to compute the operational domain of the absolute addition subnetwork input neurons.
        function self = compute_set_absolute_addition_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_absolute_addition_R_input( neuron_utilities );                                            	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute addition subnetwork output neurons.
        function self = compute_set_absolute_addition_R_output( self, Rs, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_absolute_addition_R_output( Rs, neuron_utilities );                                        	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative addition subnetwork input neurons.
        function self = compute_set_relative_addition_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_addition_R_input( neuron_utilities );                                            	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative addition subnetwork output neurons.
        function self = compute_set_relative_addition_R_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_addition_R_output( neuron_utilities );                                               % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute subtraction subnetwork input neurons.
        function self = compute_set_absolute_subtraction_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_absolute_subtraction_R_input( neuron_utilities );                                          	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute subtraction subnetwork output neurons.
        function self = compute_set_absolute_subtraction_R_output( self, Rs, s_ks, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_absolute_subtraction_R_output( Rs, s_ks, neuron_utilities );                              	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative subtraction subnetwork input neurons.
        function self = compute_set_relative_subtraction_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_subtraction_R_input( neuron_utilities );                                             % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative subtraction subnetwork output neurons.
        function self = compute_set_relative_subtraction_R_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_subtraction_R_output( neuron_utilities );                                        	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute inversion subnetwork input neurons.
        function self = compute_set_absolute_inversion_R_input( self, epsilon, delta, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_absolute_inversion_R_input( epsilon, delta, neuron_utilities );                            	% [V] Activation Domain
        ...
        % Implement a function to compute the operational domain of the absolute inversion subnetwork output neurons.
        function self = compute_set_absolute_inversion_R_output( self, c, epsilon, delta, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_absolute_inversion_R_output( c, epsilon, delta, neuron_utilities );                        	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative inversion subnetwork input neurons.
        function self = compute_set_relative_inversion_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_inversion_R_input( neuron_utilities );                                           % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative inversion subnetwork output neurons.
        function self = compute_set_relative_inversion_R_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_inversion_R_output( neuron_utilities );                                          % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute division subnetwork input neurons.
        function self = compute_set_absolute_division_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_absolute_division_R_input( neuron_utilities );                                         	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the absolute division subnetwork output neurons.
        function self = compute_set_absolute_division_R_output( self, c, alpha, epsilon, R_numerator, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_absolute_division_R_output( self, c, alpha, epsilon, R_numerator, neuron_utilities );     % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative division subnetwork input neurons.
        function self = compute_set_relative_division_R_input( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_division_R_input( neuron_utilities, neuron_utilities );                   	% [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative division subnetwork output neurons.
        function self = compute_set_relative_division_R_output( self, neuron_utilities )
            
            % Set the default input arguments.
            if nargin < 2, neuron_utilities = self.neuron_utilities; end
            
            % Compute the operational domain.
            self.R = self.compute_relative_division_R_output( neuron_utilities );                                       % [V] Activation Domain
            
        end
        
        
        % Implement a function to compute the operational domain of the relative multiplication subnetwork output neurons.
        function self = compute_set_relative_multiplication_R3( self, c, c1, c2, epsilon1, epsilon2, neuron_utilities )
            ...
            % Compute the operational domain.
            self.R = self.compute_relative_multiplication_R3( c, c1, c2, epsilon1, epsilon2, neuron_utilities );        % [V] Activation Domain
        ...
        function self = compute_set_Ileak( self, U, Gm, neuron_utilities )
            ...
            % Compute the leak current associated with this neuron.
            self.I_leak = self.compute_Ileak( U, Gm, neuron_utilities );                                                                   % [A] Leak Current
            
        end
        
        
        % Implement a function to compute and set the sodium channel current associated with this neuron.
        function self = compute_set_Ina( self, U, Gna, Am, Sm, dEm, Ah, Sh, dEh, dEna, neuron_utilities )
            ...
            % Compute the sodium channel current associated with this neuron.
            self.I_na = self.compute_Ina( U, Gna, Am, Sm, dEm, Ah, Sh, dEh, dEna, neuron_utilities );                                     % [A] Sodium Channel Current
            
        end
        
        
        % Implement a function to compute and set the total current associated with this neuron.
        function self = compute_set_Itotal( self, I_leak, I_syn, I_na, I_tonic, I_app, neuron_utilities )
            ...
            self.I_total = self.compute_Itotal( I_leak, I_syn, I_na, I_tonic, I_app, neuron_utilities );                                  % [A] Total Current
