...
%                 self.ps_mesh = self.get_cuboid_points( self.len, self.width, self.width, self.p_cm(1), self.p_cm(2), self.p_cm(3), 0, 0, 0 );
        ...
        function fig = plot_link_end_points( self, fig, plotting_options )
            ...
            plot3( self.conversion_manager.m2in( self.ps_link( 1, : ) ), self.conversion_manager.m2in( self.ps_link( 2, : ) ), self.conversion_manager.m2in( self.ps_link( 3, : ) ), plotting_options{:} )
        ...
        function fig = plot_link_com_point( self, fig, plotting_options )
            ...
            if ( nargin < 2 ) || ( isempty(fig) )
                ...
                % Create a figure to store the link com.
                fig = figure( 'Color', 'w', 'Name', 'Link COM Point' ); hold on, grid on, xlabel('x [in]'), ylabel('y [in]'), zlabel('z [in]'), title('Link COM Point')
        ...
        function fig = plot_link_mesh_points( self, fig, plotting_options )
            ...
            if ( nargin < 2 ) || ( isempty(fig) )
                ...
                fig = figure( 'Color', 'w' ); hold on, grid on, xlabel('x [in]'), ylabel('y [in]'), zlabel('z [in]'), title('Link Mesh Points')
        ...
        function fig = plot_link_points( self, fig, plotting_options, point_type )
            ...
            if ( nargin < 2 ) || ( isempty(fig) )
                ...
                fig = figure( 'Color', 'w', 'Name', 'Link Points' ); hold on, grid on, xlabel('x [in]'), ylabel('y [in]'), zlabel('z [in]'), title('Link Points')
