function [ R1_crit, R2_crit, C_crit, Kp_crit, Ki_crit ] = GetPICircuitComponents( rs, cs, Kp, Ki, bRestrictRange )
...
%Turn the desired Kp & Ki values into a vector.
Ktarget = [Kp; Ki];

%Create a grid of the available resistors and capacitors.
[Rs1, Rs2, Cs] = meshgrid( rs, rs, cs );

%Generate all possible Kp & Ki Values.
Kp = Rs2./Rs1; Ki = 1./(Rs1.*Cs);
...
for k = 1:numopts                       %Iterate through each of the resistor / capacitor options...
   ...
   %Create a vector from the current Kp & Ki values.
   Ks = [Kp(k); Ki(k)]; 
    
   %Compute an error vector for the current Kp & Ki values.
   Kerr = Ks - Ktarget;
...
Kp_crit = Kp(index); Ki_crit = Ki(index);
