...
ROAToolboxOptions.GridCellSize = [0.15; 0.15; 0.15];
...
ROAToolboxOptions.InitCircleRad = 0.4;
...
ROAToolboxOptions.IntegratorShowTime = 0.5;
...
if iscell(xs)  % element-wise
    ...
    prz = (r - 3) .* ( (r - 2) .* (r + x3) + 1.5 ); 
    x1dot =  (prz + x3) .* x1;
    x2dot =  (prz + x3) .* x2;
    x3dot =  prz .* x3 - x1 .^ 2 -x2 .^2;
