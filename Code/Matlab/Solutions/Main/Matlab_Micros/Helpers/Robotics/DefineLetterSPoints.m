...
ts = linspace(pi/4, 3*pi/2, 10);
xs1 = 0.5*cos(ts);
ys1 = 0.5*sin(ts) + 0.5;
zs1 = zeros(1, length(xs1));

%Define the second stroke.
ts = linspace(pi/2, 7*pi/4, 10);
xs2 = -0.5*cos(ts);
ys2 = 0.5*sin(ts) - 0.5;
...
dlmwrite('C:\Users\USER\Documents\Coursework\MSME\Year1\Winter2018\ME557_IntroToRobotics\Project\LetterPoints\LetterPts_S.txt', Lpts)
