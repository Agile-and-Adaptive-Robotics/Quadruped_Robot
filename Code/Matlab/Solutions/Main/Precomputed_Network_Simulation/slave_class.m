classdef slave_class
    ...
    methods
        ...
        function self = slave_class( slave_ID, muscle_ID, muscle_name, pressure_sensor_ID1, pressure_sensor_ID2, joint_ID, joint_name, measured_pressure_value1, measured_pressure_value2, measured_joint_value, desired_pressure )
            ...
            if nargin < 11, self.desired_pressure = 0; else, self.desired_pressure = desired_pressure; end

            % Define the joint value.
            if nargin < 10, self.measured_joint_value = 0; else, self.measured_joint_value = measured_joint_value; end
            
            % Define the second pressure sensor value.
            if nargin < 9, self.measured_pressure_value2 = 0; else, self.measured_pressure_value2 = measured_pressure_value2; end
            
            % Define the first pressure sensor value.
            if nargin < 8, self.measured_pressure_value1 = 0; else, self.measured_pressure_value1 = measured_pressure_value1; end
            
            % Define the joint name.
            if nargin < 7, self.joint_name = {''}; else, self.joint_name = joint_name; end
            
            % Define the joint ID.
            if nargin < 6, self.joint_ID = 0; else, self.joint_ID = joint_ID; end
            
            % Define the second pressure sensor ID.
            if nargin < 5, self.pressure_sensor_ID2 = 0; else, self.pressure_sensor_ID2 = pressure_sensor_ID2; end
            
            % Define the first pressure sensor ID.
            if nargin < 4, self.pressure_sensor_ID1 = 0; else, self.pressure_sensor_ID1 = pressure_sensor_ID1; end
