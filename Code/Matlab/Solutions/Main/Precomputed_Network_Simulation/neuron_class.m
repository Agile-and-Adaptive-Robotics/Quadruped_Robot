classdef neuron_class
    ...
    methods
        ...
        function self = neuron_class( ID, Cm, Gm, Er, R, Am, Sm, dEm, Ah, Sh, dEh, dEna, tauh_max, Gna )

            % Set the default neuron properties.
            if nargin < 14, self.Gna = 1e-6; else, self.Gna = Gna; end
            if nargin < 13, self.tauh_max = 0.25; else, self.tauh_max = tauh_max; end
            if nargin < 12, self.dEna = 110e-3; else, self.dEna = dEna; end
            if nargin < 11, self.dEh = 0; else, self.dEh = dEh; end
            if nargin < 10, self.Sh = 50; else, self.Sh = Sh; end
            if nargin < 9, self.Ah = 0.5; else, self.Ah = Ah; end
            if nargin < 8, self.dEm = 40e-3; else, self.dEm = dEm; end
            if nargin < 7, self.Sm = -50; else, self.Sm = Sm; end
            if nargin < 6, self.Am = 1; else, self.Am = Am; end
            if nargin < 5, self.R = 20e-3; else, self.R = R; end
            if nargin < 4, self.Er = -60e-3; else, self.Er = Er; end
            if nargin < 3, self.Gm = 1e-6; else, self.Gm = Gm; end
            if nargin < 2, self.Cm = 5e-9; else, self.Cm = Cm; end
