classdef joint_class
    ...
    methods
        ...
        function self = joint_class( ID, name, parent_link_ID, child_link_ID, p, R, v, w, w_screw, theta )
            ...
            if nargin < 9, self.w_screw = 0; else, self.w_screw = w_screw; end
            if nargin < 8, self.w = 0; else, self.w = w; end
            if nargin < 7, self.v = 0; else, self.v = v; end
            if nargin < 6, self.R = 0; else, self.R = R; end
            if nargin < 5, self.p = 0; else, self.p = p; end
            ...
            self.v_screw = cross( p, w_screw );
            
            % Compute the home configuration of this joint.
            self.M = [R, p; zeros(1, 3), 1];
