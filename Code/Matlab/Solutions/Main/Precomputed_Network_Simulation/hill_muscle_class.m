classdef hill_muscle_class
    ...
    methods
        ...
        function self = hill_muscle_class( ID, name, activation, activation_domain, desired_total_tension, desired_active_tension, desired_passive_tension, measured_total_tension, measured_active_tension, measured_passive_tension, tension_domain, muscle_length, resting_length, max_strain, velocity, yank, kse, kpe, b, network_dt, num_int_steps )
            
            % Set the default hill muscle properties.
            if nargin < 21, self.num_int_steps = 10; else, self.num_int_steps = num_int_steps; end
            if nargin < 20, self.network_dt = 1e-3; else, self.network_dt = network_dt; end
            if nargin < 19, self.b = 1; else, self.b = b; end
            if nargin < 18, self.kpe = 30; else, self.kpe = kpe; end
            if nargin < 17, self.kse = 30; else, self.kse = kse; end
            if nargin < 16, self.yank = 0; else, self.yank = yank; end
            if nargin < 15, self.velocity = 0; else, self.velocity = velocity; end
            if nargin < 14, self.max_strain = 0.16; else, self.max_strain = max_strain; end
            if nargin < 13, self.resting_length = 1; else, self.resting_length = resting_length; end
            if nargin < 12, self.length = self.resting_length; else, self.length = muscle_length; end
