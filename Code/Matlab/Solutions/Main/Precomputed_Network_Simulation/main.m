...
for k = 1:simulation_manager.num_timesteps                  % Iterate through each simulation time step...
    ...
    muscle_manager = muscle_manager.store_muscle_activations( simulation_manager.muscle_IDs, simulation_manager.activations(k, :) );
    
    % Compute the desired total muscle tensions associated with the current muscle activations.
    muscle_manager = muscle_manager.activations2desired_total_tensions(); 
    
    % Compute the desired active and desired passive muscle tensions associated with the current total muscle activations.
    muscle_manager = muscle_manager.desired_total_tensions2desired_active_passive_tensions(); 
    ...
    % Retrieve the sensor data from the master microcontroller.
    
    
    % Store the sensor data into the associated slave data in the slave manager.
