...
ROAToolboxOptions.GridRange = [-1, 4; -1, 4];
% A column vector specifying the size of grid cell in each dimension
%  i.e. [x1cellsize; x2cellsize; x3cellsize ...]
% ROAToolboxOptions.GridCellSize = [0.003; 0.003]; % high accuracy
ROAToolboxOptions.GridCellSize = [0.03; 0.03]; % low accuracy
...
ROAToolboxOptions.InitCircleCen = [ pi/2; pi/2 ];

%% Parameters for integrating
% End time for integrate.
ROAToolboxOptions.IntegratorTime = 30;
% Period at which intermediate results should be produced.
ROAToolboxOptions.IntegratorShowTime = 5;
...
if iscell(xs)  % element-wise
    ...
    x1dot = - sin(x1) .* (-.1 * cos(x1) - cos(x2) );
    x2dot = - sin(x2) .* (cos(x1) - .1 * cos(x2) );
