%% Relative Division After Inversion Subnetwork Derivation.

% Clear Everything.
clear, close( 'all' ), clc


%% Setup the Relative Division After Inversion Subnetwork Constraints.

% Define symbolic variables.
syms R1 R2 R3 Cm1 Cm2 Cm3 Gm1 Gm2 Gm3 gs31 gs32 dEs31 dEs32 Ia3 c1 c2 c3 delta1 delta2 U1 U2
...
% Define the desired reduced relative division after inversion subnetwork steady state output.
U3d = ( c1*R2*R3*U1 )/( c2*R1*U2 + c3*R1*R2 );

% Define the achieved reduced relative division after inversion subnetwork steady state output.
...
% Solve the first and third equations for the design constants c1 & c2.
sol = solve( [ eq1d, eq3d ], [ c1, c2 ] );

% Retrieve the design constant solutions.
c1 = simplify( sol.c1 );
c2 = simplify( sol.c2 );
