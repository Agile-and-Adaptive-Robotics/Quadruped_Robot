...
f_encode1 = @( x1 ) network_utilities.encode_reduced_absolute_inversion_input( x1 );
f_encode2 = @( x2 ) network_utilities.encode_reduced_absolute_inversion_output( x2 );
f_encode = @( Xs ) [ f_encode1( Xs( :, 1 ) ), f_encode2( Xs( :, 2 ) ) ];

% Define the decoding maps.
f_decode1 = @( U1 ) network_utilities.decode_reduced_absolute_inversion_input( U1 );
f_decode2 = @( U2 ) network_utilities.decode_reduced_absolute_inversion_output( U2 );
...
% Unpack the reduced inversion subnetwork output params.
[ c2, x2_max, R1, R2, Gna1, Gna2, dEs21, gs21, Ia2 ] = network.unpack_reduced_absolute_inversion_output_params( reduced_inversion_output_params, network.neuron_manager, network.synapse_manager, network.applied_current_manager, undetected_option );
...
U2s_desired = network.compute_encoded_desired_reduced_absolute_inversion_sso( U1s, c1, delta, x1_max, network.network_utilities );
[ U2s_achieved_theoretical, As, dts, condition_numbers ] = network.achieved_reduced_inversion_RK4_stability_analysis_encoded( U1s, Cms, Gms, Rs, Ias, gs, dEs, dt0, network.neuron_manager, network.synapse_manager, undetected_option, network.network_utilities );
...
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Desired & Achieved (Theory) SS Behavior' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Output, U2 [mV]' ), title( 'RAI: Encoded Desired & Achieved (Theory) SS Behavior' )
...
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_desired_achieved_theory_encoded' ] )

% Plot the decoded desired and achieved absolute transmission formulation results.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Desired & Achieved (Theory) SS Behavior' ); hold on, grid on, xlabel( 'Decoded Input, x [-]' ), ylabel( 'Decoded Output, y [-]' ), title( 'RAI: Decoded Desired & Achieved (Theory) SS Behavior' )
...
legend( 'Desired', 'Achieved (Theory)' )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_desired_achieved_theory_decoded' ] )

% Plot the RK4 maximum timestep vs the encoded input.
fig = figure( 'Color', 'w', 'Name', 'RAI: RK4 Maximum Timestep vs Encoded Input' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'RK4 Maximum Timestep, dt [s]' ), title( 'RAI: RK4 Maximum Timestep vs Encoded Input' )
plot( scale*Us_desired( :, 1 ), dts, '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_rk4_maximum_timestep_encoded' ] )

% Plot the RK4 maximum timestep vs the decoded input.
fig = figure( 'Color', 'w', 'Name', 'RAI: RK4 Maximum Timestep vs Decoded Input' ); hold on, grid on, xlabel( 'Decoded Input, x [-]' ), ylabel( 'RK4 Maximum Timestep, dt [s]' ), title( 'RAI: RK4 Maximum Timestep vs Decoded Input' )
plot( scale*Xs_desired( :, 1 ), dts, '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_rk4_maximum_timestep_decoded' ] )

% Plot the linearized system condition numbers vs the encoded input.
fig = figure( 'Color', 'w', 'Name', 'RAI: Condition Numbers vs Encoded Input' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Condition Number [-]' ), title( 'RAI: Condition Number vs Encoded Input' )
plot( scale*Us_desired( :, 1 ), condition_numbers, '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_condition_numbers_encoded' ] )

% Plot the linearized system condition numbers vs the decoded input.
fig = figure( 'Color', 'w', 'Name', 'RAI: Condition Numbers vs Decoded Input' ); hold on, grid on, xlabel( 'Decoded Input, x [-]' ), ylabel( 'Condition Number [-]' ), title( 'RAI: Condition Number vs Decoded Input' )
plot( scale*Xs_desired( :, 1 ), condition_numbers, '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_condition_numbers_decoded' ] )
...
if simulate_flag                            % If we want to simulate the network...
    ...
    save( [ save_directory, '\', 'reduced_absolute_inversion_subnetwork_error' ], 'xs1_input', 'Us1_input', 'Ias1_input', 'Us_achieved_numerical', 'Xs_achieved_numerical' )
...
Us2_desired = network.compute_encoded_desired_reduced_absolute_inversion_sso( Us_achieved_numerical( :, 1 ), c1, delta, x1_max, network.network_utilities );
Us2_achieved_theoretical = network.compute_encoded_achieved_reduced_inversion_sso( Us_achieved_numerical( :, 1 ), Rs( 1 ), Gms( 2 ), gs( 2, 1 ), dEs( 2, 1 ), Ias( 2 ), network.neuron_manager, network.synapse_manager, network.applied_current_manager, undetected_option, network.network_utilities );
...
header_str_encoded = 'Reduced Absolute Inversion Encoded Summary Statistics\n';
header_str_decoded = 'Reduced Absolute Inversion Decoded Summary Statistics\n';
...
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Response (Desired)' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Output, U2 [mV]' ), title( 'RAI: Encoded Steady State Response (Desired)' )
plot( scale*Us_desired( :, 1 ), scale*Us_desired( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_desired_encoded' ] )

% Create a plot of the decoded desired network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Response (Desired)' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Output, x2 [-]' ), title( 'RAI: Decoded Steady State Response (Desired)' )
plot( scale*Xs_desired( :, 1 ), scale*Xs_desired( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_desired_decoded' ] )

% Create a plot of the encoded achieved numerical network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Response (Achieved Theoretical)' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Output, U2 [mV]' ), title( 'RAI: Encoded Steady State Response (Achieved Theoretical)' )
plot( scale*Us_achieved_theoretical( :, 1 ), scale*Us_achieved_theoretical( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_achieved_theoretical_encoded' ] )

% Create a plot of the decoded achieved numerical network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Response (Achieved Theoretical)' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Output, x2 [-]' ), title( 'RAI: Decoded Steady State Response (Achieved Theoretical)' )
plot( scale*Xs_achieved_theoretical( :, 1 ), scale*Xs_achieved_theoretical( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_achieved_theoretical_decoded' ] )

% Create a plot of the encoded achieved numerical network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Response (Achieved Numerical)' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Input, U2 [mV]' ), title( 'RAI: Encoded Steady State Response (Achieved Numerical)' )
plot( scale*Us_achieved_numerical( :, 1 ), scale*Us_achieved_numerical( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_achieved_numerical_encoded' ] )

% Create a plot of the decoded achieved numerical network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Response (Achieved Numerical)' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Output, x2 [-]' ), title( 'RAI: Decoded Steady State Response (Achieved Numerical)' )
plot( scale*Xs_achieved_numerical( :, 1 ), scale*Xs_achieved_numerical( :, 2 ), '-', 'Linewidth', 3 )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_achieved_numerical_decoded' ] )

% Create a plot of the encoded desired, achieved (theory), and achieved (numerical) network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Response (Comparison)' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Output, U2 [mV]' ), title( 'RAI: Encoded Steady State Response (Comparison)' )
...
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_comparison_encoded' ] )

% Create a plot of the decoded desired, achieved (theory), and achieved (numerical) network behavior.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Response (Comparison)' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Output, x2 [-]' ), title( 'RAI: Decoded Steady State Response (Comparison)' )
...
legend( [ h1, h2, h3 ], { 'Desired', 'Achieved (Theoretical)', 'Achieved (Numerical)' }, 'Location', 'Best' )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_ssr_comparison_decoded' ] )

% Create a plot of the encoded theoretical and numerical error.
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Error' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Error, E [mV]' ), title( 'RAI: Encoded Steady State Error' )
...
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_sse_encoded' ] )

% Create a plot of the decoded theoretical and numerical error.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Error' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Error, E [-]' ), title( 'RAI: Decoded Steady State Error' )
...
legend( { 'Theoretical', 'Numerical' }, 'Location', 'Best', 'Orientation', 'Horizontal' )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_sse_decoded' ] )

% Create a plot of the encoded theoretical and numerical percentage error.
fig = figure( 'Color', 'w', 'Name', 'RAI: Encoded Steady State Error Percentage' ); hold on, grid on, xlabel( 'Encoded Input, U1 [mV]' ), ylabel( 'Encoded Error Percentage, E [%]' ), title( 'RAI: Encoded Steady State Error Percentage' )
...
legend( { 'Theoretical', 'Numerical' }, 'Location', 'Best', 'Orientation', 'Horizontal' )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_sse_percentage_encoded' ] )

% Create a plot of the decoded theoretical and numerical percentage error.
fig = figure( 'Color', 'w', 'Name', 'RAI: Decoded Steady State Error Percentage' ); hold on, grid on, xlabel( 'Decoded Input, x1 [-]' ), ylabel( 'Decoded Error Percentage, E [%]' ), title( 'RAI: Decoded Steady State Error Percentage' )
...
legend( { 'Theoretical', 'Numerical' }, 'Location', 'Best', 'Orientation', 'Horizontal' )
saveas( fig, [ save_directory, '\', 'reduced_absolute_inversion_sse_percentage_decoded' ] )
