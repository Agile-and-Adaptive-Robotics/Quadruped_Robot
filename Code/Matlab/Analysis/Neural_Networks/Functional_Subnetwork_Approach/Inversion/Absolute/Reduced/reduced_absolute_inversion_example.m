...
% c1 = 4.1e-05;                        % [-] Subnetwork Gain 1.
c1 = 1.1052631578947372e-05;                        % [-] Subnetwork Gain 1.

% delta = 1e-3;                                       % [V] Minimum Decoded Output.
% delta = 2e-3;                                       % [V] Minimum Decoded Output.
delta = 0.5e-4;                                       % [V] Minimum Decoded Output.
...
reduced_inversion_input_parameters.c1 = c1;
reduced_inversion_input_parameters.delta = delta;
reduced_inversion_input_parameters.x1_max = x1_max;
reduced_inversion_input_parameters.Gm1 = Gm1;
reduced_inversion_input_parameters.Gm2 = Gm2;
reduced_inversion_input_parameters.Cm1 = Cm1;
reduced_inversion_input_parameters.Cm2 = Cm2;
...
fig_network_encoded = figure( 'Color', 'w', 'Name', 'RAI: Encoded Input & Output vs Time' ); hold on, grid on, xlabel( 'Time, t [s]' ), ylabel( 'RAI: Encoded Input & Output, U [V]' ), title( 'RAI: Encoded Input & Output vs Time' )
plot( ts, Us( 1, : ), '-', 'Linewidth', 3 )
plot( ts, Us( 2, : ), '-', 'Linewidth', 3 )
legend( 'Encoded Input', 'Encoded Output' )
saveas( fig_network_encoded, [ save_directory, '\', 'reduced_absolute_inversion_example_encoded' ] )

% Plot the decoded network input and output over time.
fig_network_decoded = figure( 'Color', 'w', 'Name', 'RAI: Decoded Input & Output vs Time' ); hold on, grid on, xlabel( 'Time, t [s]' ), ylabel( 'RAI: Decoded Input & Output [-]' ), title( 'RAI: Decoded Input & Output vs Time' )
plot( ts, Xs( 1, : ), '-', 'Linewidth', 3 )
plot( ts, Xs( 2, : ), '-', 'Linewidth', 3 )
legend( 'Decoded Input', 'Decoded Output' )
saveas( fig_network_decoded, [ save_directory, '\', 'reduced_absolute_inversion_example_decoded' ] )

% Plot the encoded network input and output.
fig_network_encoded = figure( 'Color', 'w', 'Name', 'RAI: Decoded Output vs Decoded Input' ); hold on, grid on, xlabel( 'Encoded Input, U1 [V]' ), ylabel( 'Encoded Output, U2 [V]' ), title( 'RAI: Encoded Output vs Encoded Input' )
plot( Us( 1, : ), Us( 2, : ), '-', 'Linewidth', 3 )
saveas( fig_network_encoded, [ save_directory, '\', 'reduced_absolute_inversion_dynamic_example_encoded' ] )

% Plot the decoded network input and output.
fig_network_decoding = figure( 'Color', 'w', 'Name', 'RAI: Decoded Output vs Decoded Input' ); hold on, grid on, xlabel( 'Decoded Input [-]' ), ylabel( 'Decoded Output [-]' ), title( 'RAI: Decoded Output vs Decoded Input' )
plot( Xs( 1, : ), Xs( 2, : ), '-', 'Linewidth', 3 )
saveas( fig_network_decoding, [ save_directory, '\', 'reduced_absolute_inversion_dynamic_example_decoded' ] )
