...
L_AF = (6:0.25:7.5) * (0.0254);        % [m] Ankle flexor length (inches to meters conversion)
L_AE = (6:0.25:7.5) * (0.0254);        % [m] Ankle extensor length (inches to meters conversion)
L_KF = (7:0.5:10) * (0.0254);       % [m] Knee flexor length (inches to meters conversion)
L_KE = (7.5:0.5:10) * (0.0254);       % [m] Knee extensor length (inches to meters conversion)
...
T_KE_ankle = (l2 + l3 * cosd(60)/2) * m3 * g * cosd(60);  % [Nm] Torque from foot, at extended position
T_KE_encoder = l2 * me * g * cosd(60);                    % [Nm] Torque from encoder at ankle
Tg_KE = T_KE_leg + T_KE_ankle + T_KE_encoder;             % [Nm] Total torque on knee from gravity
...
% Setup the plot
fig = figure('Color', 'w'); hold on, grid on, xlabel('Attachment Radius [mm]'), ylabel('Required Pressure [kPa]'), title('Monoarticular Knee Flexor: Required Pressure'), ylim([350 800])
...
for n = 1:length(L_KF)                  % Iterate through each resting muscle length
    ...
    k_KF = (r_KF * pi() / 2) ./ L_KF(n);	% [-] Knee flexor strain
...
% Add the legend                             % Knee flexor plot
legend(key_KF)
...
% Setup the plot
fig = figure('Color', 'w'); hold on, grid on, xlabel('Attachment Radius [mm]'), ylabel('Required Pressure [kPa]'), title('Monoarticular Knee Extensor: Required Pressure'), ylim([350 800])
...
for n = 1:length(L_AF)                  % Iterate through each resting muscle length
    ...
    % Compute muscle strain at max flexion
    k_AF = (r_AF * pi()/2) ./ L_AF(n); % [-] Ankle extensotr strain
...
for n = 1:length(L_AE)                  % Iterate through each resting muscle length
    ...
    % Compute muscle strain at max flexion
    k_AE = (r_AE * pi()/2) ./ L_AE(n); % [-] Ankle extensotr strain
