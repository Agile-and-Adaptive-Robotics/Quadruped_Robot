%% Functional Subnetwork Analysis - Integration - Eigen Analysis - Symmetric, Without Gm
...
syms Cm gs R dEs U1 U2 lambda u B C1 C2 t

% Assume that the values are real and positive.
assume( [ Cm gs R dEs U1 U2 lambda u ], 'real' )
assume( [ Cm gs R dEs U1 U2 u ], 'positive' )

% Define the constraint equation.
dEs = -R/gs;

% Define the system flow.
f1 = ( 1/Cm )*U1 + ( -gs/( Cm*R ) )*( dEs - U1 )*U2;
f2 = ( 1/Cm )*U2 + ( -gs/( Cm*R ) )*( dEs - U2 )*U1;
