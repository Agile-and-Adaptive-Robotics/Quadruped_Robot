function PSO_results = pso_fsn_start_bot(data, n_particles, n_iterations, joint)
...
IC_data        = data.IC_data;              % Initial conditions


% Modify IC and BC data to specific joint
IC_data = IC_data(joint,:);
UB = UB(joint,:);
LB = LB(joint,:);
...
% Initialize data
pbest           = X;
pbest_err       = f_epoch_bot(pbest, 0, data, joint); %calls the function that iterates an epoch
...
for n = 1:n_iterations
    ...
    X = round(X+V, 3); %truncated to reduce computational time.
    ...
    err = f_epoch_bot(X, n+1, data, joint);
