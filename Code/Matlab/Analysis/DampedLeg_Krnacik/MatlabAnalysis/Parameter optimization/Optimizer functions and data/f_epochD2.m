...
zeta_desired           = data.zeta_desired;             % Desired zeta value for knee
omega_desired          = data.omega_desired;            % Desired omega value for knee
risetime_desired       = data.risetime_desired;         % Desired risetime value for hip
settlingtime_desired   = data.settlingtime_desired;     % Desired settlingtime value for hip
...
for m = 1:length(X)
    ...
    for n = 1:length(muscles)
        ...
        settlingtime_sim(n) = settime(t,x(:,5));
        
        
    end % end of muscle loop (n)

    % Calculate cost of hip
    cost_hip = trial_value(:,1) .* abs(risetime_sim - risetime_desired);
    
    % Calculate cost of knee
    cost_knee = trial_value(:,2) .* (100*abs( zeta_sim - zeta_desired ) + abs( omega_sim - omega_desired ));
    
    % Calculate cost of ankle
    cost_ankle = trial_value(:,3) .* abs( settlingtime_sim - settlingtime_desired );
    
    % Calculate total cost of particle
    pbest_err(m) = 400 * sum(cost_hip) + sum(cost_knee) + 100 * sum(cost_ankle);
