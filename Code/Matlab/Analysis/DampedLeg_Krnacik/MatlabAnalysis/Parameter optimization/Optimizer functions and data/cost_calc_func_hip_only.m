...
% the leg optimization of the hip only (can be used on any 3link pendulum
...
% and knee values are set, and only b and k for the hip are to be
% optimized.
...
k1_value = U(2);                  % [N/m]
mu1_value = 0;                    % [Ns/m]
b2_value = set_var(1);            % [Ns/m]
k2_value = set_var(2);            % [N/m]
mu2_value = 0;                    % [Ns/m]
...
sumdiffsq = zeros(length(muscles), 1);
...
%     w0s = [set_var(1) U(1) set_var(4)];
    ...
    %% Calculate cost by point comparison and risetime calculation for hip
    
    % Calculating error in individual points from 0 to 0.2s
    e1 = (x(1:200,1) - thetas(1:200,1));
    
    % Define cost from difference in distance at all points and joints and
    % save into array
    sumdiffsq(n) = sum(e1.^2);
   
    % Calculate the risetime for simulation and actual data. 
    risetime_x = risetime_ek(x(:,1), t);
    risetime_thetas = risetime_ek(thetas(:,1), time);

    % Save data into array
    risetime_simact(n, :) = [risetime_x risetime_thetas];
    
    %% Calculate cost for knee by comparison of calculated frequency and damping
...
% Calculate cost of hip, weighting risetime more heavily
cost_sumdiffsq = sum(sumdiffsq);
cost_risetime = 50 * sum( abs( risetime_simact(:,1) - risetime_simact(:,2) ) );

% Calculate cost of knee
cost_knee = sum(abs(omega_simact_zeta_simact(:,3) - omega_simact_zeta_simact(:,4))) + sum(abs(omega_simact_zeta_simact(:,1) - omega_simact_zeta_simact(:,2)));

cost = cost_sumdiffsq + cost_knee + cost_risetime;
