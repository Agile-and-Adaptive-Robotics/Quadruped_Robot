...
mgrs = {'T30Nmm' 'T50Nmm' 'T70Nmm' 'T100Nmm'};
T = ones(1,16);

for ii = 1:4
    mgr = mgrs{ii};
    t(1,ii) = S2L3LT4ST100I.(mgr).mgrValue/1000;
    for jj = 1:4
        trial = trials{jj};
        num = (ii-1)*4+jj;
        normCW(:,num) = S2L3LT4ST100I.(mgr).CW.(trial);
        idxCW(1,num) = find(normCW(:,num)>0,1);
        normCCW(:,num) = -S2L3LT4ST100I.(mgr).CCW.(trial);
...
title('S2L3LT4ST100I Spring Rate')
hold off
