...
mgrs = {'T40Nmm' 'T100Nmm' 'T200Nmm' 'T300Nmm'};
T = ones(1,16);

for ii = 1:4
    mgr = mgrs{ii};
    t(1,ii) = S2L4LT4ST37T.(mgr).mgrValue/1000;
    for jj = 1:4
        trial = trials{jj};
        num = (ii-1)*4+jj;
        normCW(:,num) = S2L4LT4ST37T.(mgr).CW.(trial);
        idxCW(1,num) = find(normCW(:,num)>0,1);
        normCCW(:,num) = -S2L4LT4ST37T.(mgr).CCW.(trial);
...
title('S2L4LT4ST37T Spring Rate')
hold off
