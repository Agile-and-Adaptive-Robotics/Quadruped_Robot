...
% rat leg data to be scaled to 2x time
%% BFa Trials
load BFa_trial1.mat;
load BFa_trial2.mat;
load BFa_trial3.mat;
...
%  make time values continuous instead of resetting every 1 second
for ii = 4:4:12
...
%  create single matrix to edit data
GS = zeros(length(GS_trial1(:,1)),2*length(GS_trial1(1,:)));
GS(:,1:4) = GS_trial1(:,1:4);
GS(:,5:8) = GS_trial2(:,1:4);

%  make time values continuous inGSead of resetting every 1 second
for ii = 4:4:8
    GS(:,ii-3) = GS(:,ii-3) + GS_hip_IC;         %   encoder readings GSart at 0
    GS(:,ii-2) = GS(:,ii-2) + GS_knee_IC;        %   add initial conditions to encoder readings
    GS(:,ii-1) = GS(:,ii-1) + GS_ankle_IC;       %
    
    for jj = 2:length(GS(:,1))
