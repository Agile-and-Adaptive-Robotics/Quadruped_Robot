function [du1, du2, du3, du4, du5, du6, thetabias_sym] = EOM_solver_func()
...
% friction values
D1 = mu1*M1*g*R1*sign(dtheta1(t));
D2 = mu2*M2*g*R2*sign(dtheta2(t));
D3 = mu3*M3*g*R3*sign(dtheta3(t));
...
eq_thetabias1 = k1 .* (theta1rest - theta1bias) - g .* ( M1 .* R1 .* cos(theta1rest) + M2 .* ( L1 .* cos(theta1rest) + R2 .* cos(theta2rest+ theta1rest - pi) ) + M3 .* ( L1 .* cos(theta1rest) + L2 .* cos(theta2rest+ theta1rest - pi ) + R3 .* cos( theta3rest + pi - theta1rest - theta2rest ) ) ) == 0;
eq_thetabias2 = k2 .* (theta2rest - theta2bias) - g .* ( M2 .* R2 .* cos(theta1rest - pi + theta2rest) + M3 .* (L2 .* cos( theta1rest + theta2rest - pi ) + R3 .* cos( theta3rest + pi - theta1rest - theta2rest ) ) ) == 0;
eq_thetabias3 = k3 .* (theta3bias - theta3rest) - g .* ( M3 .* R3 .* cos( theta3rest + pi - theta1rest - theta2rest ) ) == 0;
...
thetabias_sym = [theta1bias theta2bias theta3bias];
...
PE1 = M1.*g.*(r_tot - p1y) + (1/2).*k1.*( theta1(t) - theta1bias ).^2;
PE2 = M2.*g.*(r_tot - p2y) + (1/2).*k2.*( theta2(t) - theta2bias ).^2;
PE3 = M3.*g.*(r_tot - p3y) + (1/2).*k3.*( theta3(t) - theta3bias ).^2;
...
% Adding in a term for torque due to gravity.
Tg1 = - g .* ( M1 .* R1 .* cos(theta1(t)) + M2 .* ( L1 .* cos(theta1(t)) + R2 .* cos(theta2(t) + theta1(t) - pi) ) + M3 .* ( L1 .* cos(theta1(t)) + L2 .* cos(theta2(t) + theta1(t) - pi ) + R3 .* cos( theta3(t) + pi - theta1(t) - theta2(t) ) ));
Tg2 = - g .* ( M2 .* R2 .* cos(theta1rest - pi + theta2rest) + M3 .* (L2 .* cos( theta1rest + theta2rest - pi ) + R3 .* cos( theta3rest + pi - theta1rest - theta2rest ) ) );
Tg3 = - g .* ( M3 .* R3 .* cos( theta3rest + pi - theta1rest - theta2rest ) );

Tk1 = k1 .* (theta1(t) - theta1bias) + k2 .* (theta2(t) - theta2bias) + k3 .* (theta3(t) - theta3rest);
Tk2 = k2 .* (theta2(t) - theta2bias) + k3 .* (theta3(t) - theta3rest);
Tk3 = k3 .* (theta3(t) - theta3bias);

Tb1 = b1.*dtheta1(t);
Tb2 = b2.*dtheta2(t);
Tb3 = b3.*dtheta3(t);

Tf1 = mu1.*sign(dtheta1(t));
Tf2 = mu2.*sign(dtheta2(t));
Tf3 = mu3.*sign(dtheta3(t));
