%% Parameter Swarm Optimization for rat leg - Version 2
...
jdataProp = load('-mat', 'jointdyn');

% Choose damping and frequency scaling values from estimated rat data,
% using supplementary materials in Sutton et al
zeta_desired    = 0.15;                     % From figS6, Sutton et al
omega_desired   = jdataProp.omegas / 2;     % From table 1, Sutton et al

% Assuming length is the same (L1 ans L2), use same omega as L1. Zeta is
% assumed to be 0.95 (just under critically damped). Use relation between
% zeta and omega and risetime for a second order system to estimate desired
% risetime.
risetime_desired = (0.8 + 2.5 * zeta_desired) ./ omega_desired;

% Ankle scaling is more difficult. Per Sutton et al it seems like the
% general time scale between rat time scale and dog time scale is
% approximately 2, so settling time will be increased to twice as long.
settlingtime_desired = 2 * jdataProp.settling_time;
...
data.zeta_desired           = zeta_desired;         % Desired zeta value for knee
data.omega_desired          = omega_desired;        % Desired omega value for knee
data.risetime_desired       = risetime_desired;     % Desired risetime value for hip
data.settlingtime_desired   = settlingtime_desired; % Desired settlingtime value for hip
...
% and ankle
settlingtime_sim    = zeros(length(muscles), 1);
...
%%%%%%%%%%%% DYNAMIC PROPERTY CALCULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if length(pks_x) > 2
        ...
        % Calculate frequency
        omega_x = 2* pi * (0.001 * (loc_x(3) - loc_x(1)))^-1;
	...
	settlingtime_sim(m) = settime(time,thetas_ode(:,5));
    ...
    % Graph set-up
    title(sprintf('%s D2 PSO ', data_title{m}))
    xlabel('Time (s)')
...
legend('Rat hip response', 'D2 hip response', 'Rat knee response', 'D2 knee response', 'Rat ankle response', 'D2 ankle response')
    
% save results - optimized parameters and figure
save(strcat('C:\Users\krnac\OneDrive\Desktop\School\Dynamic leg\Krnacik\Parameter optimization\Results\', folder_title, '\AllTrialsPSOResults.mat'), 'PSO_results', 'thetabias_val', 'risetime_sim', 'omega_sim', 'zeta_sim', 'settlingtime_sim')
