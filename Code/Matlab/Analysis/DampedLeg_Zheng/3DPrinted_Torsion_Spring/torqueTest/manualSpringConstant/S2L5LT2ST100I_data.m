...
mgrs = {'T60Nmm' 'T100Nmm' 'T300Nmm' 'T300Nmm'};
T = ones(1,16);

for ii = 1:4
    mgr = mgrs{ii};
    t(1,ii) = S2L4LT4ST100I.(mgr).mgrValue/1000;
    for jj = 1:4
        trial = trials{jj};
        num = (ii-1)*4+jj;
        normCW(:,num) = S2L4LT4ST100I.(mgr).CW.(trial);
        idxCW(1,num) = find(normCW(:,num)>0,1);
        normCCW(:,num) = -S2L4LT4ST100I.(mgr).CCW.(trial);
        idxCCW(1,num) = find(normCCW(:,num)>0,1);
        T(1,num) = t(1,ii);
        kCW(1,num) = t(1,ii)/deg2rad(normCW(idxCW(1)+80,num));
        kCCW(1,num) = t(1,ii)/deg2rad(normCCW(idxCCW(1)+80,num));
    end
...
title('S2L4LT4ST100I Spring Rate')
hold off
