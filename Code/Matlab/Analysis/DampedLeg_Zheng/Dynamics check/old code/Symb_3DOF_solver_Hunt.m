...
KE = 0.5*I1*(x1d)^2 ;% + 0.5*M2*( v2x^2 + v2y^2) + 0.5*M3*( v3x^2 + v3y^2) + 0.5*I2*(x2d)^2 + 0.5*I3*(x3d)^2;
KE = simplify(KE);
...
% mass can reach.
PE_g = M1*g*(R1 - p1y);% + M2*g*(L1 + R2 - p2y) + M3*g*(L1 + L2 + R3 - p3y);
PE_g = simplify(PE_g);
