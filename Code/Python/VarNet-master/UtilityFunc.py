...
class UF():
    ...
    def rejectionSampling(self, func, smpfun, dof, dofT=None):
        ...
        # Ensure that dofT is None if dof is a scalar.
        if isinstance(dof, numbers.Number) and not self.isnone(dofT):                       # If the dof is a scalar and the dofT is not None...

            # Throw an error.
            raise ValueError('\'dofT\' must be None for scalar \'dof\'')

        elif isinstance(dof, numbers.Number):                                               # If the dof is a scalar...

            # Create a dof list of a single scalar.
            dof = [dof]

        # Compute the number of dofs.
        m = len(dof)

        # Ensure that dof is a list if dofT is provided.
        if m > 1 and self.isnone(dofT):                                                     # If the dof is a list and the dofT is None...
        ...
        # Evaluate the max function over the uniform grid.
        ...
        def rejecSmp(val, i):
            ...
            # Return the accepted indexes.
            return reshape(ind, nt)
            
        # Initialize a list of zero.
        ...
        while flag:
            ...
            # Compute the optimal sampling points.
            for i in range(m):                                          # Iterate through the number of dofs...

                # Retrieve the accepted samples.
                inpuTmp = smpList[i][ind[i]]                            # keep accepted samples

                # Add the current accepted samples to the previously accepted samples.
                inpuT[i] = self.vstack([inpuT[i], inpuTmp])             # add to previously accepted samples

                # Update the number of optimal samples.
                ns[i] += np.sum(ind[i])                                 # update the number of optimal samples

                # Determine whether to stop processing.
                if not flag and ns[i] < dof[i]:                         # If the number of optimal sampling points is fewer than the total number of sampling points...

                    # Set the flag to true.
                    flag = True

        # Ensure that we keep only dof samples.
        for i in range(m):                                              # Iterate through the number of dof samples...

            # Keep only the dof samples.
    ...
    def listSegment(self, vec, segdof, func=None):
        ...
        """
        This function segments a vector of values into smaller pieces stored
        ...
        # Retrieve the length of the vector to be segmented.
        n = len(vec)
        
        # Validate the input.
        if self.isnone(segdof) and self.isnone(func):                       # If the number of nodes in each segment was not provided and no function was provided...

            # Return the original vector as a list.
            return [vec]

        elif self.isnone(segdof):                                           # If the number of nodes in each segment was not provided...

            # Evaluate the function at the provided vector.
            return [func(vec, 0)]

        elif isinstance(segdof, numbers.Number):                            # If the number of nodes in each segment is a scalar...

            # Create a list of a single value using the scalar nodes in each segment.
            segdof = [segdof]

            # Define the number of segment numbers in this list to one.
            m = 1

        else:                                                               # Otherwise...

            # Define the number of segment numbers in this list.
            m = len(segdof)

        # Ensure that the number of points in the last segment does not exceed the number of vector components.
        if segdof[-1] > n:                                          # If the number of points in the last segment does not exceed the number of vector components...
        ...
        for i in range(m):                                          # Iterate through the number of points in each segment...

            # Determine how to compute the output vector.
            if not self.isnone(func):                               # If a function was not provided...

                # Evaluate the function at this vector segment.
                outVec[i] = func(vec[ind:(ind + segdof[i])][:], i)

            else:

                # Return this vector segment.
                outVec[i] = vec[ind:(ind + segdof[i])][:]

            # Define the lower index.
            ind += segdof[i]
            
        # Append the remainder of the data if it exists.
        if ind < n and not self.isnone(func):                       # If there is additional data and a function was provided...

            # Apply the given function to the remaining data.
            outVec.append( func(vec[ind:], i) )

        elif ind < n:                                               # If there is additional data and a function was not provided...

            # Return the remaining vector components.
