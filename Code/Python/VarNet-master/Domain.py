...
-------------------------------------------------------------------------------
This file provides the classes for definition and discretization of domains.
...
class Domain():
    ...
    def scaleCoord(self, x):
        ...
        # Determine whether the input dimension and domain dimension are compatible.
        if shape(x)[1] != self.dim:               # If the input and domain dimensions are not compatible.
...
class Mesh():
    ...
    def __init__(self, dim, dof, coordinates, he, bIndNum, bdof, bCoordinates, discNum = [], bDiscNum = []):
...
# ------------------------------------------------------------ DEFINE POLYGON DOMAIN 2D CLASS ------------------------------------------------------------
...
class PolygonDomain2D(Domain):
    ...
    def domPlot(self, addDescription=True, figNum=None, frameColor=None):
        ...
        # Determine how to define the frame color.
        if uf.isnone(frameColor):                           # If the frame color was not provided...
            ...
            # Duplicate the frame color.
            frameColor = [frameColor]*2

        # Determine whether to create a new figure.
        if uf.isnone(figNum):                               # If no figure was provided...
        ...
        vertices = self.vertices; vertices = np.vstack([vertices, vertices[0, :]])         # close the frame loop
        ...
        Legend = []; Legend.append('outer boundaries')
        
        # Plot the obstacles:
        for i in range(self.obsNum):                            # Iterate through each obstacle...

            # Retrieve the associated vertices.
            vertices = self.obsVertices[i]; vertices = np.vstack([vertices, vertices[0, :]])     # close the frame loop

            # Plot the associated vertices.
            plt.plot(vertices[:, 0], vertices[:, 1], frameColor[1])

            # Add an entry to the legend.
            Legend.append('obstacle {}'.format(i + 1))

        # Determine whether to add information to the plot.
        if addDescription:                                      # If we want to add a description...

            # Format the plot.
            plt.title('domain'); plt.xlabel('$x_1$'); plt.ylabel('$x_2$'); plt.legend(Legend)
    ...
    def isInside(self, x, tol=0.):
        ...
        # Ensure that the vertex dimensions are compatible with the domain dimension.
        if shape(x)[1] != self.dim:                             # If the shapes are incongruent...
        ...
        # Return whether the points are in the domain if there are no obstacles.
        if self.obsNum == 0:                                    # If there are no obstacles...

            # Return whether the points are in the domain.
            return inDom
    ...
    def getMesh(self, discNum=100, bDiscNum=50, rfrac=0, sortflg=True, discTol=None):
        ...
        It returns a Mesh object corresponding to the mesh file provided.  See Mesh
        class for details.
        """
        ...
        # Create a mesh.
        mesh = Mesh(dim = 2, dof = dof, coordinates = coordinates, he = he, bIndNum = bIndNum, bdof = bdof, bCoordinates = bCoordinates, discNum = discNum, bDiscNum = bDiscNum)                        # coordinates of inner domain # total number of boundary indicators # boundary node coordinates
...
class Domain1D(Domain):
    ...
    def __init__( self, interval = np.array([-1.0, 1.0]) ):
        ...
        # Fix the dimension to one.
        dim = 1                 # domain dimension (fixed)

        # Ensure that the interval has the correct length.
        if np.size(shape(interval)) != dim:                     # If the interval is not the correct length...
        ...
        # Set the parameters of the domain.
    ...
    def getMesh(self, discNum=100, bDiscNum=None, rfrac=0, sortflg=True, discTol=None):
        ...
        if rfrac < 0:               # If the random fraction is less than zero...

            # Set the random fraction to be zero.
            rfrac = 0

        elif rfrac > 1:             # If the random fraction is less than one...

            # Set the random fraction to be one.
            rfrac = 1
        
        # Retrieve the dimension data.
        dim = self.dim
        dof = discNum
        lim = self.lim
        ...
        # Set the spatial tolerance.
        if uf.isnone(discTol):

            # Set the tolerance to be the spatial descritization.
            tol = he                                # discretization tolerance

        else:

            # Set the tolerance to be the discretization tolerance.
            tol = np.asscalar(discTol)

        # Define the portion of points that are randomly selected.
        dof1 = math.floor(dof*rfrac)                                    # random grid

        # Define the portoin of points that are uniformly selected.
        dof2 = dof - dof1                                               # uniform grid

        # Retrieve the random points.
        coord1 = np.random.uniform(lim[0] + tol, lim[1] - tol, dof1)        # random test function locations

        # Retrieve the uniform points.
        ...
        # Determine whether it is necessary to sort the points.
        if rfrac > 0 and sortflg:                 # If there are random points and we have been asked to sort the data.

            # Sort the coordinates.
            coordinates = np.sort(coordinates)                          # sort coordinates in increasing order

        # Reshape the coordinates into a column vector.
        ...
        # Generate the mesh.
        # coordinates = coordinates of inner domain # bIndNum = total number of boundary indicators # bCoordinates = boundary node coordinates
        mesh = Mesh(dim = dim, dof = dof, coordinates = coordinates, he = he, bIndNum = bIndNum, bdof = bdof, bCoordinates = bCoordinates, discNum = discNum)
